...
export function buildFxRefreshFeedback(data: PortfolioFxRefreshResponse): FxRefreshFeedback {
  ...
  const summary = `Updated ${data.updatedCount} pairs, ${data.staleCount} still stale, ${data.errorCount} failed.`;
  if (data.staleCount > 0) {
    return {
      tone: 'warning',
      text: `Refresh attempted, but some pairs still use stale/fallback FX rates. ${summary}`,
    };
  }

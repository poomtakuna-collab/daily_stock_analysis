...
const isUnrecoverableScreenTaskError = (error: ParsedApiError) =>
  error.title === 'Screening task cannot be recovered' || error.title === '选股任务不可恢复';
...
const formatRecoverableScreenTaskPollingError = (error: ParsedApiError) => {
  ...
  if (error.category === 'upstream_network' || error.category === 'local_connection_failed') {
    return 'The screening task is still running in the background. The local service is temporarily unreachable and polling will retry automatically.';
  }
  return formatParsedApiError(error) || 'Unable to fetch screening task status for now. Polling will retry shortly.';
};
...
const formatAmount = (value: unknown) => {
  ...
  if (Math.abs(amount) >= 100_000_000) {
    return `${(amount / 100_000_000).toFixed(2)} x100M`;
  }
  if (Math.abs(amount) >= 10_000) {
    return `${(amount / 10_000).toFixed(2)} x10K`;
  }
...
const getCandidateReason = (item: AlphaSiftCandidate) => {
  ...
  return 'AlphaSift returned a candidate without a written summary. Review the factors, risk, and raw fields below.';
};
...
const summarizeAlphaSiftDiagnostic = (detail: string) => {
  if (/trade_cal returned no open trading days/i.test(detail)) {
    return 'Trading calendar has no available open days';
  }
  ...
  if (/missing .*api key|GEMINI_API_KEY|GOOGLE_API_KEY|gemini_api_key/i.test(detail)) {
    return 'Missing available LLM API key';
  }
  if (/returned no data|empty/i.test(detail)) {
    return 'No usable data returned';
  }
...
const formatScreenMessage = (value: string) => {
  ...
  if (/^LLM ranking failed/i.test(value)) {
    return `LLM reranking failed: ${summarizeAlphaSiftDiagnostic(value)}. Falling back to local factor scores.`;
  }
  ...
  if (snapshotFallback) {
    ...
    if (parsed) {
      return `Data source fallback: ${parsed.source} (${summarizeAlphaSiftDiagnostic(parsed.detail)})`;
    }
    return `Data source fallback: ${summarizeAlphaSiftDiagnostic(snapshotFallback[1])}`;
  }
...
const getHotspotStrength = (item: AlphaSiftHotspot, index: number) => {
  ...
  if (index === 0 || heat >= 90 || changePct >= 8) {
    return { label: 'Leading', className: 'bg-red-500/10 text-red-500' };
  }
  if (heat >= 80 || changePct >= 5) {
    return { label: 'Strong', className: 'bg-blue-500/10 text-blue-500' };
  }
  return { label: 'Firm', className: 'bg-cyan/10 text-cyan' };
};
...
const StockScreeningPage: React.FC = () => {
  ...
  const [taskMessage, setTaskMessage] = useState(restoredTask?.taskId ? 'Restoring screening task state...' : '');
  ...
  const alertMessages = llmDegraded
    ? screenMessages.length > 0
      ? screenMessages
      : ['LLM reranking did not complete or returned no judgement. Current candidates use AlphaSift local factor scores.']
    : screenMessages;
  ...
  const loadHotspotDetail = useCallback(async (topic: string, options: { refresh?: boolean } = {}) => {
    ...
    try {
      ...
      setHotspotDetailError(toApiErrorMessage(err, 'Failed to load theme details. Please try again later.'));
    } finally {
      if (isCurrentRequest()) {
        setLoadingHotspotDetail(false);
      }
  ...
  const loadStrategies = useCallback(async () => {
    ...
    try {
      ...
      setStrategies([]);
      setStrategyLoadError(err instanceof Error ? err.message : 'Failed to load AlphaSift strategies');
    } finally {
    ...
    function applyTaskStatus(task: AlphaSiftScreenTaskStatus) {
      ...
      if (task.status === 'completed') {
        if (task.result) {
          ...
          setError('Screening task completed but returned no candidates.');
          setCandidates([]);
          setScreenMeta(null);
        }
        finishTask();
    ...
    async function pollTask() {
      try {
        ...
        if (isUnrecoverableScreenTaskError(parsedError)) {
          setError(formatParsedApiError(parsedError) || 'Screening task cannot be restored. Submit it again.');
          setCandidates([]);
          setScreenMeta(null);
          finishTask();
          return;
    ...
    <AppPage className="max-w-6xl space-y-6 pb-12 pt-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-secondary-text">Generate stock candidates through the built-in AlphaSift adapter, then enrich them with DSA data and news</p>
          </div>
        </div>
      ...
      {!enabled ? (
        <InlineAlert
          ...
          message="Click to write ALPHASIFT_ENABLED=true. AlphaSift is installed with backend dependencies; if the adapter is missing, update dependencies or rebuild the backend."
          action={
      ...
      {enabled && !available ? (
        <InlineAlert
          ...
          message="The adapter is unavailable. Confirm backend dependencies are installed and restart the service. If needed, run pip install -r requirements.txt or use the settings/server /install endpoint."
        />
      ) : null}
      ...
      <InlineAlert
        ...
        message="AlphaSift Screening is experimental. Results are for research and decision support only, not investment advice. Markets involve risk, and trading decisions and outcomes are your responsibility."
      />
      ...
      <section className="rounded-2xl border border-border/80 bg-card/95 p-4 shadow-soft-card">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-xs leading-5 text-secondary-text">
                Powered by AlphaSift hotspot data. Strategies such as capital_heat and balanced_alpha include theme_heat in scoring.
              </p>
            </div>
          </div>
          <div className="flex flex-col items-start gap-2 lg:items-end">
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                ...
                {hotspotsExpanded ? 'Collapse hot themes' : `Expand hot themes${hotspots.length ? ` (${hotspots.length})` : ''}`}
        ...
        {!hotspotsExpanded ? (
          <div className="flex flex-col gap-2 rounded-xl border border-border/70 bg-surface/70 px-4 py-3 text-sm text-secondary-text sm:flex-row sm:items-center sm:justify-between">
            <span>
              {hotspots.length > 0
                ? `Cached ${hotspots.length} hot themes. Expand to view heat, phase, and development path.`
                : 'Hot themes are collapsed by default. Expand to read the cache; refresh fetches live data.'}
            </span>
            <span className="text-xs">Live details load after selecting a theme</span>
          </div>
        ) : hotspots.length === 0 ? (
          <div className="rounded-xl border border-dashed border-border bg-surface/70 px-4 py-6 text-sm text-secondary-text">
            Refresh fetches hot concept/industry rankings, heat score, lifecycle phase, and active leaders.
          </div>
        ) : (
          ...
          <div className="mt-4 rounded-xl border border-border/80 bg-surface/80 p-4">
            <div className="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="mt-1 text-xs leading-5 text-secondary-text">
                  {loadingHotspotDetail ? 'Loading development path and concept stocks...' : hotspotDetail?.summary || 'Click a theme to view development path and concept stocks.'}
                </p>
            ...
            {hotspotDetail && ((hotspotDetail.missingFields || []).length > 0 || (hotspotDetail.sourceErrors || []).length > 0) ? (
              <details className="mb-3 rounded-xl border border-warning/30 bg-warning/10 px-3 py-2 text-xs text-warning">
                <summary className="cursor-pointer font-semibold">Detail data is degraded. Expand to view why.</summary>
      ...
      <section className="rounded-2xl border border-cyan/35 bg-card/95 p-4 shadow-soft-card">
        <div className="mb-4 flex items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs text-secondary-text">Strategies come from AlphaSift. DSA enriches candidates with quotes, fundamentals, and news context.</p>
          </div>
        ...
        <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-4">
          {loadingStrategies ? (
            ...
            <div className="rounded-xl border border-dashed border-border bg-surface/70 p-4 text-sm text-secondary-text">
              {strategyLoadError || 'AlphaSift strategies are not loaded yet. You can enter strategy parameters manually below.'}
            </div>
          ) : (
            strategies.map((item) => {
      ...
      <section className="rounded-2xl border border-border bg-card/95 p-4 shadow-soft-card">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="grid gap-1 text-xs text-secondary-text sm:text-right">
            ...
            <span>
              Snapshots {screenMeta?.snapshotCount ?? '-'} · After filter {screenMeta?.afterFilterCount ?? '-'} · Candidates {screenMeta?.candidateCount ?? candidates.length}
            </span>
            <span>
        ...
        <div className="mb-5 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm leading-6 text-secondary-text">
              After AlphaSift returns candidates, DSA enriches top names with quotes, fundamentals, news, and auxiliary summaries.
            </p>
          </div>
                ...
                {candidates.map((item) => {
                  ...
                  const llmFallbackText =
                    llmDegraded && !llmInsightAvailable
                      ? 'LLM reranking failed or returned no judgement. Showing local factor score results.'
                      : 'No LLM judgement';
                            ...
                            <div className="grid gap-4 lg:grid-cols-[1.1fr_1fr]">
                              <div className="space-y-3">
                                ...
                                {item.dsaAnalysisSummary ? (
                                  <div>
                                    <p className="text-xs font-semibold text-secondary-text">DSA enriched summary</p>
                                ...
                                <div>
                                  ...
                                  {llmInsightAvailable ? (
                                    ...
                                    <p className="mt-1 text-xs text-secondary-text">No LLM metadata returned</p>
                                  )}
                                </div>
                                <div>

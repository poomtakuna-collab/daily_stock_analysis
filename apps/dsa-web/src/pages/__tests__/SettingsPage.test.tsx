...
vi.mock('../../components/settings', () => ({
  ...
  NotificationTestPanel: ({ items }: { items: Array<{ key: string; value: string }> }) => (
    <div>Notification test panel:{items.map((item) => item.key).join(',')}</div>
  ),
  SettingsAlert: ({
    title,
    message,
...
const baseCategories = [
  ...
  { category: 'base', title: 'Base', description: 'Base settings', displayOrder: 2, fields: [] },
  { category: 'ai_model', title: 'AI', description: 'AI models', displayOrder: 3, fields: [] },
  { category: 'notification', title: 'Notification', description: 'Notifications', displayOrder: 4, fields: [] },
  { category: 'agent', title: 'Agent', description: 'Agent', displayOrder: 5, fields: [] },
];
...
describe('SettingsPage', () => {
    ...
    getSetupStatus.mockResolvedValue({
      ...
      checks: [
        ...
        },
        {
          ...
          required: true,
          status: 'configured',
          message: 'ConfiguredModel channels。',
          nextStep: null,
        },
        {
          key: 'notification',
  ...
  it('keeps the latest first-run setup status when refresh responses resolve out of order', async () => {
      ...
      checks: [
        {
          ...
          required: true,
          status: 'needs_action',
          message: 'Stale setup status.',
          nextStep: 'This stale response should not overwrite the latest status.',
        },
      ],
    };
    ...
    expect(await screen.findByText('Initial status')).toBeInTheDocument();
    ...
    await waitFor(() => expect(screen.getByText('Latest status')).toBeInTheDocument());
    expect(screen.queryByText('Stale status')).not.toBeInTheDocument();
  ...
  it('allows brief setup smoke when only the Agent channel is incomplete', async () => {
    ...
    getSetupStatus.mockResolvedValue({
      ...
      checks: [
        {
          key: 'llm_primary',
          title: 'LLM primary channel',
          category: 'ai_model',
          required: true,
          status: 'configured',
          message: 'Claude Code CLI local generation backend is enabled (experimental/limited).',
          nextStep: null,
        },
        {
          key: 'llm_agent',
          ...
          required: true,
          status: 'needs_action',
          message: 'Agent tool calls require LiteLLM model configuration; the local CLI primary generation method is not inherited automatically.',
          nextStep: 'Configure a LiteLLM model to use the Ask-Stock Agent.',
        },
        {
          key: 'stock_list',
          title: 'Watchlist',
    ...
    await screen.findByText('1 item(s) still need action: Agent channel');
    expect(screen.getByRole('button', { name: 'Brief smoke run' })).toBeEnabled();
  ...
  it('shows missing setup items and lets the user reopen the setup check', async () => {
      ...
      checks: [
        {
          ...
          required: true,
          status: 'needs_action',
          message: 'No model channels configured yet.',
          nextStep: 'Configure model channels first.',
        },
      ],
    });
    useSystemConfigMock.mockReturnValue(buildSystemConfigState({ activeCategory: 'base' }));
    ...
    expect(screen.getByText('1 item(s) still need action: Model channels')).toBeInTheDocument();
  ...
  it('renders context compression profile labels and blank preset guidance in agent settings', () => {
        ...
        agent: [
          ...
          },
          {
            ...
            schema: {
              ...
              validation: { min: 1000 },
              displayOrder: 73,
                  description: 'Triggers summarization when estimated history tokens exceed this value; Leave blank to use the current context compression profile default.',
            },
          },
          {
            ...
            schema: {
              ...
              validation: { min: 1 },
              displayOrder: 74,
                  description: 'Keeps the most recent N user turns and following replies as raw text during compression; Leave blank to use the current context compression profile default.',
            },
          },
        ],
      },
    ...
    expect(screen.getByText('Long-context raw first')).toBeInTheDocument();
    expect(screen.getByText(/Triggers summarization when estimated history tokens exceed this value/)).toHaveTextContent('Leave blank to use the current context compression profile default.');
    expect(screen.getByText(/Keeps the most recent N user turns and following replies as raw text during compression/)).toHaveTextContent('Leave blank to use the current context compression profile default.');
  });
  ...
  it('does not render raw AlphaSift install spec in the settings card', () => {
    ...
    expect(screen.getByText('Enable the built-in experimental AlphaSift screening capability.')).toBeInTheDocument();
    expect(screen.queryByText(privateInstallSpec)).not.toBeInTheDocument();
    expect(screen.queryByText(/Install source/)).not.toBeInTheDocument();
  });
  ...
  it('scopes setup and AlphaSift helper cards to their related categories', async () => {
    ...
    expect(screen.queryByRole('heading', { name: 'AlphaSift screening' })).not.toBeInTheDocument();
    ...
    expect(await screen.findByRole('heading', { name: 'AlphaSift screening' })).toBeInTheDocument();
    expect(screen.queryByRole('heading', { name: 'First-run setup check' })).not.toBeInTheDocument();
  });
  ...
  it('renders notification test panel before notification fields', () => {
    ...
    expect(screen.getByText('Notification test panel:WECHAT_WEBHOOK_URL')).toBeInTheDocument();
  ...
  it('uses browser and backend logs in settings panel diagnostic hints outside desktop runtime', () => {
    ...
    expect(screen.getAllByText(/browser developer console and backend logs/)).toHaveLength(2);
  ...
  it('uses desktop log in settings panel diagnostic hints during desktop runtime', () => {
    ...
    expect(screen.queryByText(/browser developer console and backend logs/)).not.toBeInTheDocument();
  });
  ...
  it('renders env backup actions outside desktop runtime', () => {
    ...
    expect(screen.getByText(/Docker deployments/)).toHaveTextContent('ENV_FILE');
  });
  ...
  it('disables env backup actions when web auth is not enabled', () => {
    ...
    expect(screen.getByText(/The Web app is not protected by admin authentication/)).toBeInTheDocument();
  ...
  it('uses live auth state for env backup availability instead of loaded config items', () => {
    ...
    expect(screen.queryByText(/The Web app is not protected by admin authentication/)).not.toBeInTheDocument();
  ...
  it('shows an error when env import succeeds but reload fails', async () => {
    ...
    expect(screen.queryByText('.env backup imported and configuration reloaded.')).not.toBeInTheDocument();
  });
  ...
  it('renders desktop update notice when a newer release is available', async () => {
    desktopGetUpdateState.mockResolvedValue({
      ...
      message: 'New version 3.13.0 is available. Open GitHub Releases to download the update.',
    });
    (window as { dsaDesktop?: unknown }).dsaDesktop = createDesktopRuntime();

    render(<SettingsPage />);

    expect(await screen.findByText(/Current 3\.12\.0, latest 3\.13\.0/)).toBeInTheDocument();
  ...
  it('checks desktop updates on demand and renders the latest-version state', async () => {
    ...
    expect(await screen.findByText('Up to date:The desktop app is already up to date.')).toBeInTheDocument();
  });
  ...
  it('renders downloaded desktop update and starts install on demand', async () => {
    desktopGetUpdateState.mockResolvedValue({
      ...
      latestVersion: '3.13.0',
      releaseUrl: 'https://github.com/ZhuLinsen/daily_stock_analysis/releases/tag/v3.13.0',
      message: 'The new version has been downloaded. Restart the app to finish installation.',
      downloadPercent: 100,
    });
    (window as { dsaDesktop?: unknown }).dsaDesktop = createDesktopRuntime();
    ...
    expect(await screen.findByText('Update downloaded:The new version has been downloaded. Restart the app to finish installation.')).toBeInTheDocument();

...
const PortfolioPage: React.FC = () => {
  ...
  const loadBrokers = useCallback(async () => {
    try {
      ...
      if (brokerItems.length === 0) {
        ...
        setBrokerLoadWarning('The broker list API returned no entries, so the built-in broker list is being used (Huatai/CITIC/CMB).');
      ...
      setBrokers(FALLBACK_BROKERS);
      setBrokerLoadWarning('The broker list API is unavailable, so the built-in broker list is being used (Huatai/CITIC/CMB).');
      if (!FALLBACK_BROKERS.some((item) => item.broker === selectedBroker)) {
        setSelectedBroker(FALLBACK_BROKERS[0].broker);
      }
    }
  ...
  const loadSnapshotAndRisk = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        setRiskWarning(parsed.message || 'Risk data failed to load; showing snapshot data only.');
      }
    } catch (err) {
      setSnapshot(null);
      setRisk(null);
  ...
  const handleAnalyzePosition = async (row: FlatPosition) => {
    ...
    try {
      ...
      setPositionAnalysisMessage(`Submitted analysis task for ${row.symbol}: ${task.taskId}`);
    } catch (err) {
      setError(getParsedApiError(err));
    } finally {
  ...
  const handleTradeSubmit = async (e: React.FormEvent) => {
    ...
    if (!writableAccountId) {
      setWriteWarning('Select a specific account in the top right before entering records or importing data.');
      return;
    }
    try {
      setWriteWarning(null);
  ...
  return (
    <div className="portfolio-page min-h-screen space-y-4 p-4 md:p-6">
      ...
      <section className="grid grid-cols-1 xl:grid-cols-3 gap-3">
        <Card padding="md">
          ...
          <form className="space-y-2" onSubmit={handleTradeSubmit}>
            <input className={PORTFOLIO_INPUT_CLASS} placeholder="Stock code (for example 600519)" value={tradeForm.symbol}
            ...
            <p className="text-xs text-secondary">Fee and tax may be left empty; the system treats them as 0.</p>
        ...
        <Card padding="md">
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2 text-xs text-secondary">
              ...
              <label htmlFor="csv-dry-run">Dry run only (do not write)</label>
            </div>
            ...
            {csvCommitResult ? (
              <InlineAlert
                ...
                message={`${csvDryRun ? 'Dry-run check' : 'Actual write'}: inserted ${csvCommitResult.insertedCount}, duplicates ${csvCommitResult.duplicateCount}, failed ${csvCommitResult.failedCount}.`}
                className="rounded-lg px-3 py-2 text-xs shadow-none"
              />
            ) : null}
          </div>
          ...
          <div className="space-y-2">
            <div className="grid grid-cols-2 gap-2">
              ...
              <button type="button" className="btn-secondary text-sm" onClick={() => void loadEvents()} disabled={eventLoading}>
                {eventLoading ? 'Loading...' : 'Refresh ledger'}
              </button>
            </div>
            <div className="grid grid-cols-2 gap-2">
            ...
            <div className="text-[11px] text-secondary">
              {writeBlocked ? 'Delete corrections are available only in single-account view. Select an account first.' : 'If a ledger row is wrong, delete it and enter it again.'}
            </div>
                  ...
                  {!writeBlocked ? (
                    <button
                      ...
                      onClick={() => openDeleteDialog({
                        eventType: 'trade',
                        id: item.id,
                        message: `Delete ${item.tradeDate} ${formatSideLabel(item.side)} trade for ${item.symbol} (quantity ${item.quantity}, price ${item.price})?`,
                      })}
                    >
                      Delete
                    </button>
                  ...
                  {!writeBlocked ? (
                    <button
                      ...
                      onClick={() => openDeleteDialog({
                        eventType: 'cash',
                        id: item.id,
                        message: `Delete ${item.eventDate} cash ledger entry (${formatCashDirectionLabel(item.direction)} ${item.amount} ${item.currency})?`,
                      })}
                    >
                      Delete
                    </button>
                  ...
                  {!writeBlocked ? (
                    <button
                      ...
                      onClick={() => openDeleteDialog({
                        eventType: 'corporate',
                        id: item.id,
                        message: `Delete ${item.effectiveDate} corporate action ${formatCorporateActionLabel(item.actionType)} (${item.symbol})?`,
                      })}
                    >
                      Delete
                    </button>
                ...
                && ((eventType === 'trade' && tradeEvents.length === 0)
                  ...
                  || (eventType === 'corporate' && corporateEvents.length === 0)) ? (
                    <EmptyState
                      ...
                      description="Adjust the filters, or enter a trade, cash ledger entry, or corporate action first."
      ...
      <ConfirmDialog
        ...
        title="Delete incorrect ledger row"
        message={pendingDelete?.message || 'Delete this ledger row?'}

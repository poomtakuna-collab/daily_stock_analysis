...
export const LLM_PROVIDER_CAPABILITY_LABELS: Record<LLMProviderCapability, { label: string; hint: string }> = {
  'openai-compatible': {
    ...
    hint: 'Configure the Base URL as an OpenAI-compatible endpoint. The UI does not append /chat/completions.',
  },
  aggregator: {
    label: 'Aggregator',
    hint: 'Model visibility, routing, and pricing may vary by account permissions and platform policy.',
  },
  'official-api': {
    label: 'Official API',
    hint: 'Uses the provider official protocol or compatible entrypoint.',
  },
  'model-discovery': {
    label: 'Can fetch models',
    hint: 'Can try fetching model lists through /models; actual results still depend on account permissions and API key.',
  },
  vision: {
    label: 'Vision hint',
    hint: 'This provider is commonly used for Vision scenarios; actual capabilities depend on account and model list.',
  },
  'local-runtime': {
    label: 'Local runtime',
    hint: 'Requires the current runtime environment to access the local service.',
  },
};
...
export const LLM_PROVIDER_TEMPLATES: LLMProviderTemplate[] = [
  ...
  },
  {
    ...
    configHint:
      'The same ANSPIRE_API_KEYS value can be reused for search and LLM channels. Models and gateways below are examples; actual availability depends on account permissions and the provider console. Run a connection test first.',
    officialSources: [
  ...
  },
  {
    ...
    capabilities: ['openai-compatible'],
    configHint: 'Confirm the online inference endpoint and region; do not mix them with Coding Plan dedicated entrypoints.',
    officialSources: [
  ...
  },
  {
    ...
    capabilities: ['openai-compatible', 'model-discovery'],
    configHint: 'Model list and visibility depend on account permissions and the API key.',
  ...
  },
  {
    ...
    configHint: 'Requires the local machine, Docker, or self-hosted runner to access the Ollama service.',

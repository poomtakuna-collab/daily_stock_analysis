...
describe('LLMChannelEditor', () => {
  ...
  it('shows help dialogs for channel editor fields', async () => {
    ...
    fireEvent.keyDown(document, { key: 'Escape' });
    const capabilityHelpButton = await screen.findByRole('button', { name: 'View Runtime capability checks configuration help' });
    fireEvent.click(capabilityHelpButton);

    expect(capabilityHelpButton).toHaveAttribute('aria-expanded', 'true');
  ...
  it('hides LiteLLM wording when advanced YAML routing is enabled', () => {
    ...
    expect(screen.getByText(/Advanced model routing YAML is configured/i)).toBeInTheDocument();
    expect(screen.getByText(/Runtime primary model, fallback models, vision, and temperature are still controlled by the general fields below/i)).toBeInTheDocument();
  ...
  it('rejects bare mixed Hermes route before saving runtime generation config', async () => {
    ...
    expect(await screen.findByText(/Mixed Hermes\/non-Hermes routes are not supported for primary or fallback models/i)).toBeInTheDocument();
  ...
  it('does not test runtime-only masked Hermes secrets from the settings UI', async () => {
    ...
    const messages = await screen.findAllByText(/Runtime-injected Hermes keys are not returned to the UI/i);
  ...
  it('shows provider capability badges, official sources, and config hints', async () => {
    ...
    expect(screen.getByText('Can fetch models')).toBeInTheDocument();
    expect(screen.getByText(/Model list and visibility depend on account permissions and the API key/i)).toBeInTheDocument();
    expect(screen.getByRole('link', { name: 'OpenRouter Models API' })).toHaveAttribute(
      'href',
      'https://openrouter.ai/docs/api/api-reference/models/get-models',
    );
    expect(screen.getByText(/Capability tags are configuration references only; runtime support is not verified until tested/i)).toBeInTheDocument();
  });
  ...
  it('shows model-discovery capability for SiliconFlow provider hints', async () => {
    ...
    expect(await screen.findByText('Can fetch models')).toBeInTheDocument();
  ...
  it('does not show provider metadata for custom or unknown channels', async () => {
    ...
    expect(screen.queryByText(/Capability tags are configuration references only/i)).not.toBeInTheDocument();
  });
  ...
  it('clears active Hermes unsupported multi-key and extra-header env keys on save', async () => {
    update.mockResolvedValue({
      ...
      warnings: [
      'Cleaned unsupported Hermes Phase 3 config keys: LLM_HERMES_API_KEYS, LLM_HERMES_EXTRA_HEADERS. Hermes reserved channel supports only one LLM_HERMES_API_KEY and does not support multiple keys or extra headers. To restore previous values, use an .env backup, Git history, or desktop export backup manually; non-empty LLM_HERMES_API_KEYS / LLM_HERMES_EXTRA_HEADERS will still be rejected by backend validation.',
      ],
    });
    ...
    expect(await screen.findByText(/Cleaned unsupported Hermes Phase 3 config keys/i)).toBeInTheDocument();
    expect(screen.getByText(/To restore previous values, use an \.env backup/i)).toBeInTheDocument();
  });
  ...
  it('prompts when bare runtime models loosely match canonical OpenAI route aliases', async () => {
    ...
    await waitFor(() => {
    expect(screen.getByText('The current runtime model uses a non-canonical route alias. Select a canonical model from the dropdown.')).toBeInTheDocument();
    });
  ...
  it('shows cleanup warning and restore path after stale runtime models are removed on save', async () => {
    update.mockResolvedValue({
      ...
      updatedKeys: ['LLM_DEEPSEEK_MODELS', 'LITELLM_MODEL'],
      warnings: [
      'Removed invalid runtime model references from Primary model / Agent Primary model / Vision model / fallback models. To restore them, add the channel model list back first and reselect them; or use a desktop export backup or manual .env restore for previous LLM_* / LITELLM_MODEL / AGENT_LITELLM_MODEL / VISION_MODEL / LLM_TEMPERATURE values.',
      ],
    });
    ...
    expect(screen.getByText(/Removed invalid runtime model references/i)).toBeInTheDocument();
  ...
  it('keeps save warnings visible after onSaved-driven refresh', async () => {
    const warningMessage = 'Removed invalid runtime model references from Primary model / Agent Primary model / Vision model / fallback models.';
    const initialItems = [
      { key: 'LLM_CHANNELS', value: 'deepseek' },
      { key: 'LLM_DEEPSEEK_PROTOCOL', value: 'deepseek' },
      { key: 'LLM_DEEPSEEK_BASE_URL', value: 'https://api.deepseek.com' },
  ...
  it('shows structured troubleshooting hint when channel auth fails', async () => {
    ...
    expect(screen.getByText(/Check whether the API key is correct/i)).toBeInTheDocument();
    expect(screen.queryByText(/adjust model order or remove the unavailable model/i)).not.toBeInTheDocument();
  });
  ...
  it('shows tested model and model-availability hints when a model is disabled', async () => {
    ...
    expect(screen.getByText(/Basic connection tests use the first model in the list by default: deepseek-ai\/DeepSeek-V3/i)).toBeInTheDocument();
    expect(screen.getByText(/Basic connection tests only test the first model in the list by default/i)).toBeInTheDocument();
    expect(screen.getByText(/adjust model order or remove the unavailable model/i)).toBeInTheDocument();
    expect(screen.getByText(/model is enabled, visible to the account/i)).toBeInTheDocument();
    expect(screen.queryByText(/proxy, TLS/i)).not.toBeInTheDocument();
  ...
  it('shows focused quota exceeded troubleshooting hints', async () => {
    ...
    expect(await screen.findByText(/provider reported exhausted quota/i)).toBeInTheDocument();
    expect(screen.queryByText(/adjust model order or remove the unavailable model/i)).not.toBeInTheDocument();
  });
  ...
  it('keeps manual model input available when discovery fails', async () => {
    ...
    expect(screen.getByText(/Automatic model discovery is currently provided only for OpenAI Compatible \/ DeepSeek channels/i)).toBeInTheDocument();
  ...
  it('maps discovery format errors to the /models troubleshooting hint', async () => {
    ...
    expect(screen.getByText(/This channel returned an incompatible \/models response. Enter the model list manually./i)).toBeInTheDocument();
  });
  ...
  it('maps discovery empty responses to the /models troubleshooting hint', async () => {
    ...
    expect(screen.getByText(/The channel \/models endpoint returned no usable model IDs/i)).toBeInTheDocument();
    expect(screen.queryByText(/Try another compatible model or disable extra response modes/i)).not.toBeInTheDocument();
  });

...
const RUNTIME_CAPABILITY_OPTIONS: Array<{ value: LLMCapabilityCheck; label: string; hint: string }> = [
  { value: 'json', label: 'JSON', hint: 'Checks whether response_format JSON output works.' },
  { value: 'tools', label: 'Tools', hint: 'Checks whether function/tool calling works.' },
...
const RUNTIME_ONLY_HERMES_SECRET_MESSAGE = 'Runtime-injected Hermes keys are not returned to the UI. To test from settings, re-enter the key or save it to .env.';
      ...
      {expanded ? (
        <div className="settings-surface-overlay-soft space-y-4 px-4 py-4">
          ...
          <Input
            ...
            placeholder={
              channel.protocol === 'gemini' || channel.protocol === 'anthropic'
                ? 'Official APIs can leave this blank'
          ...
          {showProviderTemplateDetails ? (
            <div className="space-y-2 rounded-xl border border-[var(--settings-border)] bg-[var(--settings-surface-hover)] p-3">
              ...
              <p className="text-[11px] leading-5 text-muted-text">
                Capability tags are configuration references only; runtime support is not verified until tested.
              </p>
            </div>
          ) : null}
          ...
          <Input
            ...
            placeholder={channel.protocol === 'ollama' ? 'Local Ollama can leave this blank' : 'Multiple keys can be comma-separated'}
          />
          </div>
          ...
          <div className="space-y-3 rounded-xl border border-[var(--settings-border)] bg-[var(--settings-surface-hover)] p-3">
            ...
            <Input
              ...
              hint={
                discoveredModels.length > 0
                  ? 'If a custom model name is not listed, add it manually. Saved format remains comma-separated.'
          ...
          <div className="flex items-center gap-2 pt-1">
            ...
            {testState?.text ? (
              <div className="space-y-1">
                ...
                {selectedModels[0] ? (
                  <p className="text-[11px] text-secondary-text">
                    Basic connection tests use the first model in the list by default: {selectedModels[0]}
                  </p>
                ) : null}
                {testState.hint ? (
                  <p className="text-[11px] text-secondary-text">
            ...
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                ...
                <p className="mt-0.5 text-[11px] text-secondary-text">
                  Sends real LLM requests only when triggered manually. Multiple checks may take 20-40 seconds.
                </p>
              </div>
              <Button
                type="button"
...
function buildModelOptions(models: string[], selectedModel: string, autoLabel: string): Array<{ value: string; label: string }> {
  ...
  if (selectedModel && !models.includes(selectedModel)) {
    options.push({ value: selectedModel, label: `${selectedModel} (current setting)` });
  }
...
const LLM_TROUBLESHOOTING_HINTS: Record<string, string> = {
  auth: 'Check whether the API key is correct, has no extra spaces, and whether this channel requires organization or project permissions.',
  timeout: 'Retry is possible. If timeouts continue, check the Base URL, proxy, provider region, or local firewall.',
  quota: 'Check balance, plan quota, RPM/TPM limits, or concurrency settings. Retry later if needed.',
  model_not_found: 'Confirm the model name matches the channel protocol, and use Fetch models to check the actual available model list.',
  empty_response: 'The channel connected but returned no body. Try another compatible model or disable extra response modes before testing again.',
  network_error: 'Check the Base URL, proxy, TLS/certificates, gateway, or local network policy, then retry later.',
  invalid_config: 'Complete protocol, Base URL, API key, and model settings before running the test.',
  unsupported_protocol: 'Automatic model discovery is currently provided only for OpenAI Compatible / DeepSeek channels. Maintain the model list manually.',
};

const LLM_REASON_HINTS: Record<string, string> = {
  missing_api_key: 'The API key is empty, or comma-separated values contain no usable key. Enter at least one valid key before testing.',
  api_key_rejected: 'The provider rejected this API key. Check the key, organization/project permissions, region, and account status.',
  rate_limit: 'The provider hit RPM/TPM or concurrency limits. Reduce request frequency or retry later.',
  insufficient_balance: 'The provider reported insufficient balance, billing, or quota. Check account balance and plan status.',
  quota_exceeded: 'The provider reported exhausted quota. Check account plan, remaining quota, and project limits.',
  provider_blocked: 'The request was blocked by the provider or gateway. Check account risk controls, regional restrictions, model permissions, gateway policy, content policy, or request origin limits.',
  dns_error: 'DNS resolution failed. Check the Base URL domain, proxy, and DNS settings.',
  tls_error: 'TLS/certificate handshake failed. Check HTTPS certificates, gateways, or corporate proxy policy.',
  ...
  provider_prefix_mismatch: 'The model provider prefix does not match this channel. Confirm whether this model should use this channel OpenAI-compatible route.',
  capability_unsupported: 'The current model or compatibility layer does not support this capability. Basic text connection may still work; switch models or disable the capability dependency.',
};
...
function getLlmTroubleshootingHint(
  ...
  if (code === 'format_error') {
    return context === 'discovery' || stage === 'model_discovery'
      ...
      : 'The response shape does not match expectations. Confirm this channel is compatible with the Chat Completions API.';
  }
  if (code === 'empty_response' && (context === 'discovery' || stage === 'model_discovery')) {
    return 'The channel /models endpoint returned no usable model IDs. Check whether the Base URL points to a compatible model-list endpoint, or enter models manually.';
  }
...
function buildLlmTestHint(result: {
  ...
  const scopeInfo = 'Basic connection tests only test the first model in the list by default.';
  ...
  const modelActionHint = shouldSuggestModelListChange
    ? 'If this model is unavailable, adjust model order or remove the unavailable model and retry.'
    : '';
...
export const LLMChannelEditor: React.FC<LLMChannelEditorProps> = ({
  ...
  const handleSave = async () => {
    ...
    if (hasEmptyName) {
      setSaveMessage({ type: 'local-error', text: 'Channel name is required and can contain only letters, numbers, or underscores.' });
      return;
    }
    ...
    if (managesRuntimeConfig) {
      ...
      if (mixedPrimary || mixedFallback) {
        setSaveMessage({ type: 'local-error', text: 'Mixed Hermes/non-Hermes routes are not supported for primary or fallback models. Choose either Hermes-only or non-Hermes-only routes.' });
        return;
      }
    ...
    if (managesRuntimeConfig) {
      ...
      if (invalidFallbackModel) {
        setSaveMessage({ type: 'local-error', text: 'Some fallback models are invalid. Select them again.' });
        return;
      }
      ...
      if (invalidVisionModel) {
        setSaveMessage({ type: 'local-error', text: 'The current Vision model cannot use a Hermes deployment. Select a non-Hermes route.' });
        return;
      }
    }
      ...
      setCapabilityStates((previous) => ({
        ...
        [channel.id]: {
          ...
          text: Object.keys(capabilityResults).length > 0
            ...
            : result.success
              ? 'No capability check result returned'
              : buildLlmFailureText(result),
      ...
      >
        <div className="space-y-1">
          ...
          <p className="text-xs text-muted-text">
            Add provider channels to fetch model lists automatically and select multiple models, or keep entering models manually. Configuration syncs to the .env file.
          </p>
        </div>
      ...
      {!isCollapsed ? (
        <div className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
          <div className="rounded-[1.35rem] border border-[var(--settings-border)] bg-[var(--settings-surface)] p-4 shadow-soft-card">
            <div className="mb-3 flex items-center justify-between">
              <div>
                ...
                <p className="mt-1 text-xs text-secondary-text">Select a provider preset, then create a draft configuration.</p>
              </div>
          ...
          {managesRuntimeConfig ? (
            <div className="rounded-[1.35rem] border border-[var(--settings-border)] bg-[var(--settings-surface)] p-4 shadow-soft-card">
              ...
              <div className="mb-4">
                ...
                <p className="mt-1 text-[11px] text-secondary-text">
                  Controls model randomness. 0 is deterministic, 2 is maximum randomness, and 0.7 is recommended.
                </p>
              </div>
              ...
              {availableModels.length === 0 ? (
                <div className="rounded-xl border border-dashed settings-border-strong settings-surface-overlay-soft px-3 py-2 text-xs text-muted-text">
                  Add at least one enabled channel with models before primary/fallback/Vision options appear.
                </div>
              ) : (
                <div className="space-y-4">
                  <div>
            ...
            <InlineAlert
              variant="warning"
              message="Advanced model routing YAML is configured. This page only manages channel entries and basic connection information. Runtime primary model, fallback models, vision, and temperature are still controlled by the general fields below. If YAML parsing succeeds, its routes and available model declarations take precedence; this configuration does not overwrite the YAML file."
